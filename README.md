...
- [merge_sort.py](merge_sort.py): Merge Sort (CLRS) — O(n log n) em todos os casos
...
```bash
python sort_cli.py --algorithm merge_sort --type random --size 100 --show-input --debug
...
- `--algorithm`: `merge_sort` | `quicksort` | `heapsort` | `insertionsort`
...
python runner.py --algorithm merge_sort --type random --size 1000 --repeat 3
...
- `--algorithm`: `insertion`, `merge_sort`, `heapsort`, `quicksort` ou qualquer módulo que exporte `sort(A, debug=False)`
...
1. **Insertion Sort** — O(n²) | melhor para pequenos vetores ou semi-ordenados
2. **Merge Sort** — O(n log n) garantido | estável, divide-e-conquista
3. **Heapsort** — O(n log n) | in-place, sem espaço extra
4. **Quicksort** — O(n log n) average / O(n²) worst | in-place, mais rápido na prática
